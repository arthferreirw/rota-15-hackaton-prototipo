...
export const DesktopHeader: React.FC = () => {
  ...
  // Dynamic Route Breadcrumb Mapping
  const routeDetailsMap: Record<string, { group: string; title: string }> = {
    '/': { group: 'PRINCIPAL', title: 'Visão Geral & Métricas' },
    '/financeiro': { group: 'GESTÃO', title: 'Gestão Financeira' },
    '/rentabilidade': { group: 'GESTÃO', title: 'Análise de Rentabilidade' },
    '/estoque': { group: 'GESTÃO', title: 'Controle de Estoque' },
    '/delivery': { group: 'DELIVERY', title: 'Mapa de Entregas & Logística' },
    '/comparar-precos': { group: 'COMPRAS', title: 'Comparador de Preços' },
    '/fornecedores': { group: 'COMPRAS', title: 'Rede de Fornecedores' },
    '/compra-coletiva': { group: 'COMPRAS', title: 'Compras Coletivas (Cluster C08)' },
    '/oportunidades': { group: 'INTELIGÊNCIA', title: 'Oportunidades & Economia' },
    '/mapa': { group: 'INTELIGÊNCIA', title: 'Mapa Interativo de Viçosa' },
    '/simulador': { group: 'INTELIGÊNCIA', title: 'Simulador de Margem & Custos' },
    '/configuracoes': { group: 'SISTEMA', title: 'Configurações do Restaurante' }
  };

  const currentRoute = routeDetailsMap[location.pathname] || { group: 'PLATAFORMA', title: 'Rota 15' };
  ...
  return (
    <header className="hidden md:flex items-center justify-between gap-4 px-6 py-3 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-xs min-h-[64px]">
      {/* Left Area: Dynamic Breadcrumb & Search Bar */}
      <div className="flex items-center gap-4 min-w-0 flex-1">
        ...
        <div className="hidden lg:flex items-center gap-2 text-xs font-bold text-slate-500 shrink-0">
          <span className="text-slate-400 uppercase tracking-wider text-[10px] font-extrabold">
            {currentRoute.group}
          </span>
          <ChevronRight size={13} className="text-slate-300" />
          <h1 className="text-slate-900 font-extrabold text-sm tracking-tight truncate">
            {currentRoute.title}
          </h1>
        </div>
        ...
        <div className="relative flex-1 max-w-sm" ref={searchRef}>
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Buscar fornecedores, cotações, pedidos..."
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div className="hidden xl:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold shadow-2xs shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 border border-emerald-200/60 text-xs font-extrabold shrink-0" title="Rede Viçosa Ativa">
        ...
        <div className="relative shrink-0" ref={notificationRef}>
          ...
          >
            <Bell size={19} />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#FF3131] text-white text-[9px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-xs">
                {unreadCount}
              </span>
            )}
          </button>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <div className="w-8 h-8 rounded-xl bg-[#FF862F]/10 text-[#FF862F] flex items-center justify-center text-base font-bold shadow-2xs shrink-0">
